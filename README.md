...
the test suite and the model-annotation check — as one timed, pass/fail report. It reads the
services already on your machine rather than starting any, and tells you what to start if one
is down. System tests are the one job it leaves out by default, since they are much the
