...
- **Dependabot PRs don't get the repo's Actions secrets.** `secrets.RAILS_MASTER_KEY` resolves to an empty string on any workflow run triggered by Dependabot (GitHub keeps a separate Dependabot secret store), so `Rails.application.credentials` decrypts to nothing and anything reading a credential fails only on those PRs while every other branch stays green. That's how the JWT API tests broke on the redis 6.0.0 bump (`JWT::DecodeError: HMAC key expected to be a String`, 10 errors). Reproduce locally with `mv config/master.key /tmp/ && RAILS_MASTER_KEY= bin/rails test`. Fixed by making the app not need the credential in **development and test only** — `Api::JwtAuthenticatable.secret_key` derives a domain-separated key from `Rails.application.key_generator` when `Rails.env.local?`, and raises `MissingSecretKey` in every other environment rather than silently signing tokens with a different key. Don't widen that fallback to production: it would both hide a credentials misconfiguration and couple API token forgery to `secret_key_base`, which is otherwise independent of the JWT key. The Dependabot secret store was deliberately left empty rather than given a copy of the master key, which would hand it to workflow runs that install unreviewed gem versions.
