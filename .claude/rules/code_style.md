...
- The ban covers hand-written prose. The `# == Schema Information` block above
  each `app/models/*.rb` class is generated by `annotaterb` (see
  [`technical_stack.md`](technical_stack.md)) — never hand-edit or delete one.
  It regenerates on `bin/rails db:migrate`, so a hand-edit comes straight back;
  to refresh it deliberately, run `bundle exec annotaterb models`.
