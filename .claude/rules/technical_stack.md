...
- **Model schema annotations**: `annotaterb` 4.24.0 (`group :development`, `require: false` like `brakeman`/`bundler-audit` — the rake task requires it explicitly and nothing needs it at boot). The maintained fork of `annotate`, which last shipped in 2021 and breaks on Rails 7.1+. Writes the `# == Schema Information` column list above each `app/models/*.rb` class, introspecting the **live database** rather than `db/schema.rb` — the gem offers no option to read the schema file. `.annotaterb.yml` deliberately carries **only the six keys that differ from the gem's defaults**: `exclude_tests`, `exclude_fixtures`, `exclude_factories`, `exclude_serializers` (all `true`, which is what restricts annotation to models) and `show_indexes: false` / `show_foreign_keys: false` (both on by default; they restate `db/schema.rb` verbatim, and indexes and foreign keys are read and reviewed there and in the migrations, not from a model header). `bin/rails g annotate_rb:config` dumps all ~62 keys at their resolved defaults, which buries the ones that matter. `lib/tasks/annotate_rb.rake` hooks `db:migrate`/`db:rollback` so annotations can't drift locally; skip with `ANNOTATERB_SKIP_ON_DB_TASKS=1`. Regenerate by hand with `bundle exec annotaterb models`; CI's `test` job re-runs it and fails on any diff, so a stale annotation can't merge. The blocks are generated, never authored — see the carve-out in [`code_style.md`](code_style.md)'s otherwise-absolute "no comments in code" rule.
