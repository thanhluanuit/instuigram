...
- **API documentation / contract tests**: rswag 2.17 (`rswag-specs` + `rspec-rails` in `development, test`; `rswag-api` and `rswag-ui` top-level, since they mount engines that serve `/api-docs` at runtime). RSpec is scoped to `spec/requests/api/v1` and exists only because rswag's DSL requires it — see [`testing.md`](testing.md). `bundle exec rspec` runs the 13 contract examples; `RSWAG_DRY_RUN=0 bundle exec rake rswag` regenerates the committed `swagger/v1/swagger.yaml` from real executed responses (plain `rake rswag` defaults to `RSWAG_DRY_RUN=1`, which collects metadata *without* issuing requests). CI runs `bundle exec rspec` for the pass/fail signal and plain `rake rswag` (dry-run) for the drift check — the dry-run document is byte-identical to the executed one, so regenerating it in CI costs no second spec run. Every response schema sets `additionalProperties: false`, so a field *added* to a controller fails the specs too, not just a field removed. Note the released 2.17 generator writes `spec/swagger_helper.rb`, not the `spec/openapi_helper.rb` that rswag's unreleased master produces, while the config keys inside it are already the `openapi_*` spellings.
...
- **Coverage**: SimpleCov (`SimpleCov.command_name "Minitest"` + `SimpleCov.start "rails"` in `test/test_helper.rb`, before any app code loads — the explicit command name is needed because `rspec-rails` is loaded in the test env, and SimpleCov otherwise auto-detects the run as "RSpec"; the rswag specs deliberately don't start SimpleCov, so they can't clobber the report), report-only — generates `coverage/index.html` locally on every `bin/rails test` run and prints the summary % to CI's `test` job log; no enforced minimum yet.
